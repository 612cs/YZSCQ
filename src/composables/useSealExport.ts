...
export function useSealExport() {
  const exportPng = async (renderResult: SealRenderResult, companyName: string, templateName: string) => {
    const blob = await exportSvgToPngBlob(renderResult.svgMarkup, renderResult.width, renderResult.height)
